...
def summarize_resume(text):
    try:
        response = ollama.chat(model='llama3', messages=[
            {
                "role": "user", 
                "content": 'The following text is an applicant\'s resume. I want you to sort it and summarize it into the following categories: skills, experiences, projects, awards. Your output should have a very brief format, with the list of skills listed after skills. The list of experiences with brief descriptions should be listed after experiences. The list of projects with brief descriptions should be listed after the projects title. and a list of the awards should be given. Nothing else should be output. Don\'t give any introduction before giving this output, all I need is this output and nothing else. If you cannot find information for any of these fields, label it n/a. Here is the resume: ' + text,
            },
        ])
...
def compare_with_job_description(skills, experiences, projects, awards, job_description, key_responsibilities, skillset_requirements):
    try:
        comparison_input = f"Skills:\n{skills}\n\nExperiences:\n{experiences}\n\nSkillset Requirements:\n{skillset_requirements}\n\nJob Description:\n{job_description}\n\nKey Responsibilities:\n{key_responsibilities}\n\nProjects:\n{projects}\n\nAwards:\n{awards}"
        response = ollama.chat(model='llama3', messages=[
            {
                "role": "user", 
                "content": 'The following text contains an applicant\'s skills, experiences, projects, and awards, along with a job description and key responsibilities. I want you to compare them and provide a score from 0 to 100 for how well the skills match the skillset requirements and how well the experiences, projects, and awards match the job description and key responsibilities. Don\'t disadvantage an applicant who is overqualified or has more experience than is needed for the position. Do a thorough analysis before coming up with your final scores. Look at the awards hollistically when deciding look at the prestige of the award and the associated names. Provide four separate scores: one for skills, one for experiences, one for projects, and one and awards. Your output should be in the format: "Skills: A, Experiences: B, Projects: C, and Awards: D". Nothing else should be outputed, absolutely nothing else. Here is the comparison data: ' + comparison_input,
            },
        ])
        return response['message']['content']
    except httpx.RequestError as e:
...
pdf_path = '/Users/krishnasuresh/Desktop/SWE_Resume.pdf'
...
try:
    ...
    if extracted_text:
        ...
        if summarized_text:
            ...
            project_questions = generate_project_questions(parsed_data["Projects"])
            if project_questions:
                print("Project Questions:\n", project_questions)
            else:
                print("Failed to generate project questions.")
        else:
